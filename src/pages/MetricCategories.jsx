...
export default function MetricCategories() {
  ...
  const handleSearch = async () => {
    ...
    const res = await searchMetricCategories(params)
    if (res.status == 200) {
      const data = await res.data
      setMetricCategories(data.content || [])
  ...
  const save = async () => {
    if (editing) {
      await updateMetricCategory(editing.id, form)
    } else {
      await createMetricCategory(form)
    }
    setOpen(false)
    load()
  }
  ...
  return (
    <div>
        ...
        <table className="table w-full">
          ...
          <tbody>
            ...
            {metricCategories.length === 0 && (
              <tr>
                <td colSpan="6" className="text-center text-gray-500 py-4">No metricCategories found</td>
              </tr>
            )}
          </tbody>
        </table>
     ...
     {/* Pagination Controls */}
      ...
      {open && (
        <Modal
          title={editing ? 'Edit MetricCategory' : 'New MetricCategory'}
          ...
          <div className="space-y-3">
              ...
              <div className="flex justify-end">
                <button
                    ...
                    title="Save MetricCategory Data"
                  >
